...
class Ejecutor(object):
	def __init__(self,listaParametros,codSolic,rutaArchivo):
			#print "Metodo Constructor del Ejecutor Tasacion Preliminar Detalle General"
			try:
				...
				SolicDetGralTasaPre = ejecutarQuery(query)
				SolicDetGralTasaPre = SolicDetGralTasaPre[0]
				print SolicDetGralTasaPre

				query = "SELECT * FROM ICX_TRAFICO WHERE "
				query += "F_CDR BETWEEN '" + str(listParam["Fecha Desde"])+ "' AND '" +str(listParam["Fecha Hasta"])
				query += "' AND TAS_CASO_TRAFICO_OPERADORA = '" + str(listParam["Operadora"])
				query += "' AND TAS_CASO_TRAFICO_DIR_CONTABLE = " + str(listParam["Direccion_Contable"])
				query += " AND TAS_LISTA_PRECIO_MONEDA = " + str(listParam["Moneda"])
				query += " AND C_TIPO_CDR = '" + str(SolicDetGralTasaPre["C_TIPO_CDR"]) + "' "
				...
				nombreReporte+= "_"+ SolicDetGralTasaPre["DIRECCION_CONTABLE"].replace(" ","_") + "_" + str(fecha) +".txt"

				reporte = open(rutaArchivo + nombreReporte,"w")

				reporte.write("REPORTE DE |TASACION PRELIMINAR |DETALLADO GENERAL" + "\r\n")

				reporte.write("Operadora|Tipo CDR|Direccion Contable|Moneda" + "\r\n")
 				valores = str(listParam["Operadora"])+"|"+str(SolicDetGralTasaPre["C_TIPO_CDR"])+"|"
 				valores += str(SolicDetGralTasaPre["DIRECCION_CONTABLE"])+"|"+str(SolicDetGralTasaPre["Moneda"])
				...
				listParam["tituloCorreo"] = listParam["tituloCorreo"].replace("[/C_DIRECCION_CONTABLE]",str(SolicDetGralTasaPre["DIRECCION_CONTABLE"]))
