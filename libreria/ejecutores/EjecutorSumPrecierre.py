...
class Ejecutor(object):
	def __init__(self,listaParametros,codSolic,rutaArchivo):
			#print "Metodo Constructor del Ejecutor Sumarizado Precierre"
			try:
				...
				query +="A.F_DESDE and A.F_HASTA THEN 'N' ELSE 'Y' END AS Derivado, date_format(B.F_CDR,'%Y%m%d') as diaRemanente"
				query +=" FROM  ICX_SOLIC_PRECIERRE A INNER JOIN ICX_PRECIERRE B ON A.C_SOLICITUD = B.C_SOLICITUD "
				...
				query +="WHERE B.C_SOLICITUD = " + str(listaParametros["Cod Solicitud Precierre"]) #+ str(codSolic)
				...
				hojaExcel1["A1"] = "REPORTE DE TASACIÓN PRECIERRE SUMARIZADO"
				...
				hojaExcel1.cell(column=5, row= 2).value = "Solicitud Precierre"

				hojaExcel1.cell(column=1, row=3).value = solicPreCierre["C_OPERADORA"]
				hojaExcel1.cell(column=2, row=3).value = solicPreCierre["C_TIPO_CDR"]
				hojaExcel1.cell(column=3, row=3).value = solicPreCierre["C_DIRECCION_CONTABLE"]
				hojaExcel1.cell(column=4, row=3).value = solicPreCierre["AB_MONEDA"]
				hojaExcel1.cell(column=5, row=3).value = solicPreCierre["C_SOLICITUD"]
				...
				nombreReporte+= str(solicPreCierre["F_HASTA"].strftime('%Y%m%d'))+ "_" + solicPreCierre["C_DIRECCION_CONTABLE"].replace(" ","_") + "_" + str(fecha) +".xlsx"
				#nombreReporte = nombreReporte.replace(" ","_")
				#print solicPreCierre
