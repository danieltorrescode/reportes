...
class Ejecutor(FuncionesComunes):
	def __init__(self,listaParametros,codSolic,rutaArchivo):
			#print "Metodo Constructor del Ejecutor Precierre Detallado Segun Formato 111"
			try:
				...
				nombreReporte+=  "_" + solicPrecierre["C_DIRECCION_CONTABLE"].replace(" ","_") + "_"+ str(fecha) +"_CTRANS.txt"
				...
				nombreReporte2 = "ICX_"+str(solicPrecierre["C_SOLICITUD"]) +"_"+ solicPrecierre["C_OPERADORA"]
				nombreReporte2+= "_FDESDE_" + str(solicPrecierre["F_DESDE"].strftime('%Y%m%d'))+ "_FHASTA_"
				nombreReporte2+= str(solicPrecierre["F_HASTA"].strftime('%Y%m%d'))
				nombreReporte2+=  "_" + solicPrecierre["C_DIRECCION_CONTABLE"].replace(" ","_") + "_"+ str(fecha) +"_SIN_CTRANS.txt"
				reporte_SIN_CTRANS = open(rutaArchivo + nombreReporte2,"w")
				'''
				camposCabecera = "REPORTE DE TASACION PRECIERRE DETALLADO SEGUN FORMATO 111" + "\r\n"
				...
				query += "INNER JOIN ICX_SOLIC_PRECIERRE C ON C.C_SOLICITUD = "+ str(solicPrecierre["C_SOLICITUD"])
				...
				for reg in ejecutarQuery_v2(50000,query):
					...
					linea += self.troncal(reg["IA_TC"],reg["IA_ROUTE_IN_EXT"],reg["IA_ROUTE_OUT_EXT"],
											reg["PREP_RUTA_ENT_OPERADORA"],solicPrecierre["C_OPERADORA"]) #+ "|"
